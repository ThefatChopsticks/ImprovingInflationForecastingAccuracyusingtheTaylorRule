...
ggplot(df2)+ 
  geom_line(aes(x= date, y = error, col = "recon"))+ 
...
```{r}
mydata %<>% mutate(
  newbaseA = baseA - as.numeric(epsilon[1]),
  newbaseB = baseB - as.numeric(epsilon[2]),
  newbaseC = baseC - as.numeric(epsilon[3])) 
```
...
#unbias X 
uX <- mydata %>% 
  filter(year_quarter>= "1969Q1" & year_quarter <= "2016Q1") %>%
  select(newbaseA, newbaseB,newbaseC)
```
...
train <- mydata %>% 
  filter(year_quarter>= "1969Q1" & year_quarter <= "2020Q1") %>% 
  select(newbaseA, newbaseB,newbaseC) %>% 
  mutate (tot = newbaseA + newbaseB +newbaseC ) %>% 
  select(tot,newbaseA,newbaseB,newbaseC)

realised_bts %<>% 
  mutate(
    r_tot = realised_baseA + realised_baseB + realised_baseC
  )
```
...
```{r}
nbts %<>% mutate(
  newABC = newbaseA + newbaseB + newbaseC,
  newAB = newbaseA + newbaseB,
  newC = newbaseC
) %>%
  select(newABC,newbaseA ,newbaseB, newbaseC )

realised_bts %<>% mutate(
  newABC = realised_baseA + realised_baseB + realised_baseC,
  newAB = realised_baseA + realised_baseB,
  newC = realised_baseC
) %>%
  select(newABC,realised_baseA ,realised_baseB, realised_baseC)
```
...
forecast_error <- (realised_bts - nbts)

W<- var(forecast_error)

solve(t(U)%*%W%*%U)

S%*%(J-J%*%W%*%U%*%solve(t(U)%*%W%*%U)%*%t(U))

reconciled<- t(S%*%(J-J%*%W%*%U%*%solve(t(U)%*%W%*%U)%*%t(U))%*%t(nbts)) 

rcon_ABC <- reconciled[,1] 
rcon_A <- reconciled[,2]
rcon_B <- reconciled[,3]
rcon_C <- reconciled[,4]
...
reconciled <- cbind(year_quarter,rcon_ABC,rcon_A,rcon_B,rcon_C)


mydata <- full_join(mydata, reconciled, by = c('year_quarter'= 'year_quarter')) 
```
...
#test  "2016Q2" to "2019Q3"

bts <- mydata %>% 
  filter(year_quarter >= "2016Q2" & year_quarter<= "2019Q3") %>%
  select(baseA, baseB, baseC)
...
epsilon = bts - realised_bts 

epsilon %<>% summarise(biasA  = mean(baseA), 
                      biasB = mean(baseB),
                      biasC = mean(baseC))
```
...
bts %<>% mutate(
  testnewbaseA = baseA - as.numeric(epsilon[1]),
  testnewbaseB = baseB - as.numeric(epsilon[2]),
  testnewbaseC = baseC - as.numeric(epsilon[3])
) 
year_quarter<- mydata %>%  
  filter(year_quarter>= "2016Q2" & year_quarter <= "2019Q3")  %>%
  select(year_quarter)

newbase <- cbind(year_quarter,bts%>% select(testnewbaseA,testnewbaseB,testnewbaseC))

mydata <- full_join(mydata,newbase, by =c("year_quarter"="year_quarter"))
...
testnbts <- mydata %>% 
  filter(year_quarter >= "2016Q2" & year_quarter<= "2019Q3") %>%
  select(testnewbaseA, testnewbaseB, testnewbaseC)

 data1<- mydata %>% 
  filter(year_quarter >= "2016Q2" & year_quarter<= "2019Q3")

 ggplot(data1)+ 
   geom_line(aes(x= year_quarter , y = newbaseA), colour = "blue") + 
   geom_line(aes(x= year_quarter , y = baseA, colour = "red"))
 
 ggplot(data1)+ 
   geom_line(aes(x= year_quarter , y = newbaseC), colour = "blue") + 
   geom_line(aes(x= year_quarter , y = baseC), colour = "red")
 
testnbts %<>% mutate(
  testnewABC = testnewbaseA + testnewbaseB + testnewbaseC) %>%
  select(testnewABC, testnewbaseA, testnewbaseB, testnewbaseC)
 
 
 
testreconciled<- t(S%*%(J-J%*%W%*%U%*%solve(t(U)%*%W%*%U)%*%t(U))%*%t(testnbts)) 


testrcon_ABC <- testreconciled[,1] 
testrcon_A <- testreconciled[,2]
testrcon_B <- testreconciled[,3]
testrcon_C <- testreconciled[,4]

newbase <- cbind(year_quarter,testrcon_ABC,testrcon_A,testrcon_B,testrcon_C)
...
```{r}
nbts %<>% mutate(
  new_baseA = newbaseA + 0.95*newbaseC,
  new_baseB = newbaseB + 0.05*newbaseC,
  newAB = new_baseA + new_baseB
)%>% 
  select(newAB, new_baseA, new_baseB)
...
realised_bts %<>% mutate(
  ...
  select(newAB, realised_baseA ,realised_baseB)
...
n<-t(s%*%(j-j%*%w%*%u%*%solve(t(u)%*%w%*%u)%*%t(u))%*%t(nbts))
...
```{r, warning=F}
data1 %>% 
  select(year_quarter, baseC, newbaseC) %>%
  gather(Forecast, interest, - year_quarter) %>%
  ggplot()+ 
  geom_line(aes(x= year_quarter, y = interest, col = Forecast))+ 
  ylab("Intereste Rate")+ 
  xlab("Year by Quarters")
```
...
test_A<- mydata %>%
  filter(year_quarter>= "2016Q2" & year_quarter <= "2019Q3") %>% 
  select( testrcon_A,realised_baseA,testnewbaseA)

var(test_A$testnewbaseA-test_A$realised_baseA)-var(test_A$testrcon_A-test_A$realised_baseA)

(mean((test_A$testnewbaseA-test_A$realised_baseA)^2)) - (mean((test_A$testrcon_A-test_A$realised_baseA)^2))

mean(abs(test_A$testnewbaseA-test_A$realised_baseA))- mean(abs(test_A$testrcon_A-test_A$realised_baseA))
...
test_B<- mydata %>%
  filter(year_quarter>= "2016Q2" & year_quarter <= "2019Q3") %>% 
  select( testrcon_B,realised_baseB,testnewbaseB)


((mean((test_B$testnewbaseB - test_B$realised_baseB)^2)))

var(test_B$testnewbaseB-test_B$realised_baseB)-var(test_B$testrcon_B-test_B$realised_baseB)

((mean((test_B$testnewbaseB -test_B$realised_baseB)^2^0.5)))-((mean((test_B$testrcon_B-test_B$realised_baseB)^2^0.5)))

mean(abs(test_B$testnewbaseB-test_B$realised_baseB))- mean(abs(test_B$testrcon_B-test_B$realised_baseB))
```
...
test_C<- mydata %>%
  filter(year_quarter>= "2016Q2" & year_quarter <= "2019Q3") %>% 
  select( testrcon_C,realised_baseC,testnewbaseC)

var(test_C$testnewbaseC-test_C$realised_baseC)-var(test_C$testrcon_C-test_C$realised_baseC)

(mean((test_C$testnewbaseC-test_C$realised_baseC)^2)) - (mean((test_C$testrcon_C-test_C$realised_baseC)^2))
```
...
mydata %>%
  filter(year_quarter>= "2016Q2" & year_quarter <= "2019Q3") %>% 
  ggplot()+ 
  geom_line(aes(x= year_quarter, y=realised_baseA- testnewbaseA, colour = "red"))+ 
  geom_line(aes(x= year_quarter, y=realised_baseA- testrcon_A, colour = "pink"))+
  xlab("Year by Quarters") + 
  ylab("Interest Rate")+
  theme_test() 
```
...
a<- mydata %>%
  filter(year_quarter>= "2016Q2" & year_quarter <= "2019Q3") %>% 
  ggplot() + 
  geom_line(aes(x= year_quarter, y=testnewbaseA, col= "Unbiased Forecast A"))+ 
  geom_line(aes(x= year_quarter, y=testrcon_A, col= "Reconciled forecast A"))+  
  xlab("Year by Quarters") + ylab("Interest Rate") +
  theme_test() + 
...
mydata %>%
  filter(year_quarter>= "2016Q2" & year_quarter <= "2019Q3") %>% 
  ggplot() + 
  geom_line(aes(x= year_quarter, y=testnewbaseA -testrcon_A, col= "Unbiased Forecast A"))+ 
  xlab("Year by Quarters") + ylab("Interest Rate") +
  theme_test() + 
  theme(legend.position = "top")
```
...
b <- mydata %>%
  filter(year_quarter>= "2016Q2" & year_quarter <= "2019Q3") %>% 
  ggplot() + 
  geom_line(aes(x= year_quarter, y=testnewbaseB, col= "Unbiased forecast B"))+ 
  geom_line(aes(x= year_quarter, y=testrcon_B, col= "Reconciled forecast B"))+  
  xlab("Year by Quarters") + ylab("Interest Rate") +
  theme_test() + 
  theme(legend.position = "top") 
...
c<- mydata %>%
  filter(year_quarter>= "2016Q2" & year_quarter <= "2019Q3") %>% 
  ggplot() + 
  geom_line(aes(x= year_quarter, y=testnewbaseC, col= "Unbiased forecast C"))+ 
  geom_line(aes(x= year_quarter, y=testrcon_C, col= "Reconciled forecast C"))+  
  xlab("Year by Quarters") + ylab("Interest Rate") +
  theme_test() + 
  theme(legend.position = "top")
```
